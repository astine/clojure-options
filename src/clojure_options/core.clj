     ...
     (loop [tokens tokens
       ...
       (let [token (first tokens)]
           (cond (empty? token)
                 (if (empty? errors)
                   output
                   (throw (ex-info (reduce str errors) output)))
                 (= token "--")
                 (if (empty? errors)
                   (reduce reducer output (map vector (rest tokens) (repeat :free)))
                   (throw (ex-info (reduce str errors) (reduce reducer output (map vector (rest tokens) (repeat :free))))))
                 (= token "-")
                 ...
                 (cond (some #{\=} token)
                       (let [[option parameter] (clojure.string/split token #"=")]
                         (cond (parameter-options (subs option 2))
                               ...
                               (recur (rest tokens) output (str "Warning: Used '=' with option that doesn't take a parameter: '" option "'"))
                               :else (recur (rest tokens) output (str "Warning: Invalid option '" option "'"))))
                       (parameter-options (subs token 2))
                       (if (#{"--" "-" nil} (second tokens))
                         (recur (rest tokens) (reducer output [(subs token 2) nil]) (str "Warning: No parameter for option '" token "'"))
                       ...
                       :else (recur (rest tokens) output (str "Warning: Invalid option '" token "'")))
